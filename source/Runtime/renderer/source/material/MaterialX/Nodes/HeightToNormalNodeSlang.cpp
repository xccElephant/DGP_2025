...
/// Name of function to compute sample size in uv space. Takes uv, filter size,
/// and filter offset as input, and return a 2 channel vector as output
...
void HeightToNormalNodeSlang::createVariables(
    const ShaderNode&,
    GenContext&,
    Shader&) const
{
    // Default filter kernels from ConvolutionNode are not used by this derived
    // class.
}

void HeightToNormalNodeSlang::computeSampleOffsetStrings(
    const string& sampleSizeName,
    const string& offsetTypeString,
    unsigned int,
    StringVec& offsetStrings) const
{
    // Build a 3x3 grid of samples that are offset by the provided sample size
    for (int row = -1; row <= 1; row++) {
        for (int col = -1; col <= 1; col++) {
            offsetStrings.push_back(
                " + " + sampleSizeName + " * " + offsetTypeString + "(" +
...
void HeightToNormalNodeSlang::emitFunctionDefinition(
    const ShaderNode&,
    GenContext& context,
    ShaderStage& stage) const
{
    DEFINE_SHADER_STAGE(stage, Stage::PIXEL)
    {
        ...
        shadergen.emitLibraryInclude(
            "stdlib/genslang/lib/mx_sampling.slang", context, stage);
        shadergen.emitLineBreak(stage);
    }
}

void HeightToNormalNodeSlang::emitFunctionCall(
    ...
    DEFINE_SHADER_STAGE(stage, Stage::PIXEL)
    {
        ...
        if (!inInput || !scaleInput) {
            throw ExceptionShaderGenError(
                "Node '" + node.getName() +
                "' is not a valid heighttonormal node");
        }
        ...
        shadergen.emitLine(
            "float " + sampleName + "[" + std::to_string(sampleCount) + "]",
            stage);
